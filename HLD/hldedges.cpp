...
void edge_update(int x, int y, int z,
    ...
    segment.range_update(0, 0, segment.tree.size() / 4 - 1, positions[x] + 1, positions[y], z);
}

int edge_query(int x, int y,
               Segment_Tree &segment,
               vector<int> &head,
               vector<int> &positions,
               vector<int> &parents,
    ...
    while (head[x] != head[y]) {
        if (depth[head[x]] < depth[head[y]]) swap(x, y);
        result += segment.query(0, 0, segment.tree.size()/4 - 1, positions[head[x]], positions[x]);
        x = parents[head[x]];
    }
    ...
    // Skip LCA
    result += segment.query(0, 0, segment.tree.size() / 4 - 1, positions[x] + 1, positions[y]);
    return result;
}
...
int32_t main(){
    ...
    while (q--){
        ...
        if (operation == 1){
            cin >> z;
            edge_update(x, y, z, segment, head, positions, parents, depth);
        } else if (operation == 2){
            cout << edge_query(x, y, segment, head, positions, parents, depth) << "\n";
        }
    }
